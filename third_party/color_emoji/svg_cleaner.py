...
class SvgCleaner(object):
  ...
  class _Writer(object):
    ...
    def _to_text(self, node, lines, indent):
      """Node is a node generated by _Reader, either a string or a tuple. Lines is
      a list to collect the lines of output Indent is the indentation level
      for this node."""
      ...
      if isinstance(node, _Text_Node):
        ...
        if node.contents:
          ...
          for elem in node.contents:
            self._to_text(elem, lines, indent + 1)
          line = [margin]
          line.append('</%s>' % node.name)
          lines.append(''.join(line))
        else:
